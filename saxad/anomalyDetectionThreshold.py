...
def hardcoded_threshold(output_model:np.array, output_real:np.array, threshold:float, amount_anamalous_features: int):
    ...
    for timestamp_sample in g:
       ...
       if true_counter_dict.get("True", 0) >= math.ceil(len(timestamp_sample)*amount_anamalous_features):
           print("Timestamp is anamolous! {} features have a weird value".format(true_counter_dict.get("True", 0)))
    return g

def hmu_anomaly_score_gaussian_tail_probability_threshold(model_output, real_output, window_size, short_window_size): 
    raw_anomaly_scores = np.absolute(model_output - real_output)
    hmu_likelihood_scores = []
    for i in range(0, len(hmu_likelihood_scores)):
        current_window = raw_anomaly_scores[i+(window_size-1):(i+(window_size*2-1))]
        ...
        q_Function = lambda x: 0.5 - 0.5*special.erf(x/np.sqrt(2)) #q_function is the complementary of the error function 
        hmu_likelihood_scores.append(1 - q_Function((short_window_mean-window_mean)/window_variance))
